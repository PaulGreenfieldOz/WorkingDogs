...
These kMer filters are simply sets of kMers tiled from a set of reference sequences, and are generated by BuildFilter. Some pre-built filters are available via the GHAP distribution (https://doi.org/10.4225/08/59f98560eba25).  
...
## Installing and building FilterReads
...
FilterReads is ‘installed’ simply by copying the desired code file to an appropriate directory on your system. For example, if you want to 
run the self-contained .Net8 FilterReads code file on a Linux system compatible with Ubuntu 24 (glibc 2.39), you should copy the 
FilterReads file from the Linux64DN8FDU24 directory.You may have to set permission bits on the FilterReads code file before you can execute it on Linix. 

Compilation scripts are provided for both Windows (.ps1) and Linux (.sh). For example, `BuildFilterReads_Linux64DN8AOTU24.sh` 
will build a stand-alone FilterReads executable targeting .NET8. This script was run on Ubuntu 24 to build the executable, and the resulting FilterReads code will be 
...
The FilterReads code itself is in FilterReads.cs in the FilterReads directory, and you'll also need to download 
kMers.cs, SeqFiles.cs and Sequence.cs to the WorkingDocsCoreLibrary. The FilterReads compilation scripts are intended to be run from the 
WorkingDogs/FilterReads directory.
...
You can build FilterReads for other target systems, such as MacOS on Arm, by creating a 
.pubxml file with the appropriate RuntimeIdentifier - see https://learn.microsoft.com/en-us/dotnet/core/rid-catalog. 
The command `dotnet publish ./FilterReads.sln -c release /p:PublishProfile=Linux64DN10AOTFolderProfile.pubxml` 
in the `BuildFilterReads_Linux64DN8AOTU24.sh` script is what builds the
framework-dependent x64 Linux FilterReads executable, and other versions can be built by changing the name of the profile file in this 
publish command.nt x64 Linux FilterReads executable, and other versions can be built by changing the name of the profile file in the 
publish command.
